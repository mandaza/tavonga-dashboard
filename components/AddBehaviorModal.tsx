...
import { X, Plus, Loader2, AlertTriangle, Camera, Video, MapPin, Clock, Activity, User } from 'lucide-react'
...
export default function AddBehaviorModal({ isOpen, onClose, onSuccess }: AddBehaviorModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await apiClient.createBehavior(payload)
      toast.success('Behavior log created successfully!')
      onSuccess()
      handleClose()
    } catch (error: any) {
      console.error('Error creating behavior:', error)
      toast.error(error.message || 'Failed to create behavior log')
    } finally {
      ...
      <div className="bg-white rounded-[20px] w-full max-w-5xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-semibold text-[#000000]">Add Behavior Log</h2>
              <p className="text-sm text-[#6D6D6D]">Record a new behavior incident</p>
            </div>
          </div>
          <button
            onClick={handleClose}
